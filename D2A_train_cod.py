...
from models.D2ANet import D2ANet
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, gts, edges) in enumerate(train_loader, start=1):
            ...
            images = images.cuda()
            gts = gts.cuda()
            # print(f"[Train] Step {i}, GT min/max/mean: {gts.min().item():.4f} / {gts.max().item():.4f} / {gts.mean().item():.4f}")
            edges = edges.cuda()
            s1, s2, s3, edge_map = model(images)
            # s1 = model(images)
            ...
            if edge_map is not None:
                loss_dice = dice_loss(edge_map, edges)
                bce_iou_deep_supervision = bce_iou1 + bce_iou2 + bce_iou3 + 3*loss_dice
            else:
                bce_iou_deep_supervision = bce_iou1 + bce_iou2 + bce_iou3
        ...
        if epoch > 20:
            torch.save(model.state_dict(), save_path + 'D2ANet_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'D2ANet_epoch_{}.pth'.format(epoch + 1))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'D2ANet_epoch_best.pth')
